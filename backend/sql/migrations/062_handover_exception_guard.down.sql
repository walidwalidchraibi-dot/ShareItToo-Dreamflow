  ...
  IF EXISTS (
    ...
    SELECT 1 FROM support_cases
     WHERE case_type = 'trust_safety'
       AND case_subtype = 'offplatform_deposit_request'
  ) THEN
    RAISE EXCEPTION '062 rollback refused: handover exception evidence exists';
  END IF;
END;
$$;

DROP TRIGGER audit_log_handover_exception_guard ON audit_log;
DROP FUNCTION sit_validate_handover_exception_audit();
DROP INDEX audit_log_handover_exception_request_idx;
