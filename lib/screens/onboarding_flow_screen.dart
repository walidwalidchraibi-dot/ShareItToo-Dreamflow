                ...
                child: GestureDetector(
                  ...
                  child: Column(
                      ...
                      children: [
                        ...
                        Text('Miete alles. Von Leuten in deiner Nähe.',
...
class _OnboardingFlowScreenState extends State<OnboardingFlowScreen> {
  ...
  Map<int, double> get _imageOffsetXMap =>
      _imageOffsetXBySlide ??= <int, double>{};
  Map<int, double> get _imageOffsetYMap =>
      _imageOffsetYBySlide ??= <int, double>{};
  Map<int, double> get _imageScaleMap => _imageScaleBySlide ??= <int, double>{};
  Map<int, double> get _legacyAlignXMap =>
      _legacyAlignXBySlide ??= <int, double>{};
  Map<int, double> get _legacyAlignYMap =>
      _legacyAlignYBySlide ??= <int, double>{};
  bool get _isPlacementMode => _placementMode ??= false;

  bool get _hasAnyBackgroundImages =>
      _slides.any((s) => (s.imagePath ?? '').trim().isNotEmpty);
  ...
  static const _slides = <_OnboardingSlideData>[
    ...
    _OnboardingSlideData(
      ...
      body:
          'Vermiete Werkzeuge, Technik, Outdoor-Gear, Küchenmaschinen, Fahrräder, Schmuck und vieles mehr – oder miete genau das, was dir gerade fehlt.',
      bodyMaxLines: 3,
    ...
    _OnboardingSlideData(
      ...
      body:
          'Verifizierte Profile, Bewertungen, ein integrierter Chat sowie eine bilddokumentierte Übergabe und Rückgabe geben dir Sicherheit – beim Mieten und Vermieten.',
      bodyMaxLines: 3,
      titleAlign: TextAlign.center,
    ),
  ];
  ...
  void _schedulePersistPlacement(int slideIndex, Offset offset, double scale) {
    ...
    _persistDebounce = Timer(const Duration(milliseconds: 650), () async {
      try {
        ...
        if ((prevX - offset.dx).abs() < 0.4 &&
            (prevY - offset.dy).abs() < 0.4 &&
            (prevScale - scale).abs() < 0.003) return;
        final prefs = await SharedPreferences.getInstance();
  ...
  Future<void> _persistPlacementNow(int slideIndex) async {
    try {
      _persistDebounce?.cancel();
      final offset = Offset(_imageOffsetXMap[slideIndex] ?? 0.0,
  ...
  Widget build(BuildContext context) {
    ...
    final pageView = PageView(
      ...
      physics: (_hasAnyBackgroundImages && _isPlacementMode)
      ...
      children: List.generate(_slides.length, (i) {
        ...
        final effectivePlacementMode =
            _hasAnyBackgroundImages ? _isPlacementMode : false;
        return _OnboardingPage(
          ...
          legacyAlignment: (legacyAlignX == null || legacyAlignY == null)
              ? null
              : Alignment(legacyAlignX, legacyAlignY),
          ...
          onScaleChanged: (nextScale) {
            ...
            final offset =
                Offset(_imageOffsetXMap[i] ?? 0.0, _imageOffsetYMap[i] ?? 0.0);
      ...
      children: [
        ...
        if (_hasAnyBackgroundImages && _isPlacementMode)
          Positioned(
            right: 16,
            bottom:
                (_showBottomActions ? 132 : 24) + (_showPageIndicator ? 22 : 0),
            child: SafeArea(
              child: _OnboardingZoomControls(
                visible:
                    (_slides[_currentPage].imagePath ?? '').trim().isNotEmpty,
              ...
              if (_showPageIndicator)
                Padding(
                  padding: EdgeInsets.fromLTRB(
                      16, 2, 16, _showBottomActions ? 0 : 10),
                  child: Align(
                ...
                Padding(
                  ...
                  child: AnimatedSwitcher(
                    ...
                    child: _currentPage < 2
                        ? Column(
                            key: const ValueKey('actions_default'),
                            children: [
                                _NavRow(
                                  left: _SecondaryCta(
                                      icon: Icons.arrow_back,
                                      label: 'Zurück',
                                      onTap: _currentPage == 0 ? null : _back),
                                  right: _PrimaryCta(
                                ...
                                const SizedBox(height: 10),
                                _TextLinkCta(
                                    label: 'Überspringen', onTap: _skip),
                              ])
                        : Column(
                            key: const ValueKey('actions_auth'),
                            children: [
                                _NavRow(
                                  left: _SecondaryCta(
                                      icon: Icons.arrow_back,
                                ...
                                const SizedBox(height: 10),
                                _TextLinkCta(
                                    label: 'Schon ein Konto? Anmelden',
                                    onTap: _goLogin),
          ...
          child: BackdropFilter(
            ...
            child: Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                _ZoomButton(
                    icon: Icons.remove, onTap: onZoomOut, enabled: canZoomOut),
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Icon(icon,
            size: 18,
            color: Colors.white.withValues(alpha: enabled ? 1.0 : 0.55)),
      ),
    );
  }
}
...
class _SplashScreenState extends State<_SplashScreen>
  ...
  late final Animation<double> _scale = TweenSequence<double>([
    TweenSequenceItem(
        ...
        weight: 1),
    TweenSequenceItem(
        tween: Tween<double>(begin: 1.08, end: 0.84)
            .chain(CurveTween(curve: Curves.easeInOutCubic)),
        weight: 1),
  ]).animate(_controller);
...
class _OnboardingPage extends StatelessWidget {
  ...
  const _OnboardingPage(
      ...
      required this.preserveOriginalSize,
      required this.defaultImageAlignment,
      required this.legacyAlignment,
      required this.imageOffsetX,
      required this.imageOffsetY,
      required this.imageScale,
      required this.placementMode,
      required this.canZoomOut,
      required this.avoidCropping,
      required this.baseFitCover,
        ...
        return LayoutBuilder(
          builder: (context, constraints) {
            ...
            final needsMigration =
                (dx == null || dy == null || dx.isNaN || dy.isNaN);
            if (needsMigration) {
              ...
              scheduleMicrotask(
                  () => onPlacementChanged(Offset(fallbackDx, fallbackDy)));
            }
            ...
            return Stack(
              ...
              children: [
                ...
                const Positioned.fill(
                    child:
                        IgnorePointer(child: _OnboardingReadabilityOverlays())),
                // Foreground content.
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: [
                ...
                Color.lerp(secondary, BrandColors.logoGradientEnd, 0.55)!
          ...
          child: Stack(
            children: [
              ...
              Positioned(
                ...
                top: 80,
                child: _GlowBlob(
                    color: BrandColors.logoAccent.withValues(alpha: 0.38),
                    size: 360),
              ),
              Positioned(
                left: -80,
                bottom: -160,
                child: _GlowBlob(
                    color: Colors.white.withValues(alpha: 0.18), size: 420),
              ),
              Positioned(
                right: -120,
                bottom: -220,
                child: _GlowBlob(
                    color: primary.withValues(alpha: 0.24), size: 520),
              ),
            ],
          ),
        ),
...
class _OnboardingBackgroundImage extends StatefulWidget {
  ...
  final ValueChanged<double>? onScaleChanged;
  const _OnboardingBackgroundImage(
      {required this.imagePath,
  ...
  @override
  State<_OnboardingBackgroundImage> createState() =>
      _OnboardingBackgroundImageState();
}

class _OnboardingBackgroundImageState
    extends State<_OnboardingBackgroundImage> {
  Offset? _lastFocal;
  double? _startScale;
  ...
  void _resolveAssetSize(String? path) {
    ...
    try {
      ...
      final listener = ImageStreamListener((info, _) {
        ...
        debugPrint(
            '[Onboarding] Failed to resolve asset image size for $path: $error');
      });
      _imageStreamListener = listener;
      stream.addListener(listener);
    } catch (e) {
      debugPrint(
          '[Onboarding] Failed to start resolving asset image size for $path: $e');
    }
  }
  ...
  Offset _clampOffset(
    ...
    return Offset(
        clampedDx.isNaN ? 0.0 : clampedDx, clampedDy.isNaN ? 0.0 : clampedDy);
  }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      final emphasisScale =
          widget.avoidCropping ? 1.0 : (1.01 + (0.02 * widget.emphasis));
      ...
      final baseScale = widget.preserveOriginalSize || !widget.baseFitCover
          ? 1.0
          : math.max(
              viewportW / assetLogical.width, viewportH / assetLogical.height);
      ...
      return GestureDetector(
        ...
        onScaleUpdate: (d) {
          ...
          final allowPan = widget.placementMode ||
              (_startPointerCount ?? d.pointerCount) >= 2;

          // Scale: pinch to zoom (persisted user scale; containScale is applied on top)
          final nextUserScale =
              (startScale * d.scale).clamp(minUserScale, maxUserScale);
          ...
          final nextClampedCurrentOffset = _clampOffset(
              offset: widget.offset,
              viewport: viewport,
              assetLogical: assetLogical,
              effectiveScale: nextEffectiveScale);
          if (nextClampedCurrentOffset != widget.offset)
            widget.onOffsetChanged(nextClampedCurrentOffset);
          ...
          if (allowPan) {
            ...
            final proposed = Offset(
                widget.offset.dx + deltaPx.dx, widget.offset.dy + deltaPx.dy);
...
class _BlurWipeRevealState extends State<_BlurWipeReveal>
    with SingleTickerProviderStateMixin {
          ...
          child: BackdropFilter(
            ...
            child: Container(
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 8),
                Expanded(
                    child: Text('Ziehen oder pinchen zum Zoomen',
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: theme.textTheme.labelSmall?.copyWith(
      ...
      InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
                color:
                    Colors.white.withValues(alpha: canZoomOut ? 0.14 : 0.08)),
          ),
          child: Icon(Icons.remove,
              size: 18,
              color: Colors.white.withValues(alpha: canZoomOut ? 1.0 : 0.55)),
        ),
      ),
      const SizedBox(width: 10),
      InkWell(
...
class _SecondaryCta extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      splashFactory: NoSplash.splashFactory,
      overlayColor: WidgetStateProperty.all(
          Colors.white.withValues(alpha: enabled ? 0.05 : 0.0)),
      borderRadius: BorderRadius.circular(16),
      child: Container(
        height: 52,
        decoration: BoxDecoration(
...
class _DotsIndicator extends StatelessWidget {
  ...
  const _DotsIndicator(
      {required this.currentIndex, required this.count, this.emphasis = 1.0});
    ...
    return Row(
      ...
      children: List.generate(count, (i) {
        ...
        return AnimatedContainer(
          ...
          height: 9,
          decoration: BoxDecoration(
            color: (isActive ? active : inactive).withValues(
                alpha: (isActive ? 1.0 : 1.0) * (0.82 + (0.18 * emphasis))),
            borderRadius: BorderRadius.circular(999),
          ),
        );
      }),
