...
class _AccountSettingsScreenState extends State<AccountSettingsScreen> {
    ...
    await showGeneralDialog<void>(
      ...
      pageBuilder: (ctx, a1, a2) {
        return AccountDeletionDialog(
          ...
          leftAction: AccountDeletionDialogAction(label: 'Abbrechen', onPressed: () => Navigator.of(ctx, rootNavigator: true).maybePop()),
      ...
      transitionBuilder: (ctx, anim, secondary, child) {
        ...
        return Opacity(opacity: anim.value, child: Transform.scale(scale: 0.96 + (0.04 * t), child: child));
      },
    );
  }
        ...
        return StatefulBuilder(
          builder: (ctx, setLocalState) {
            return AccountDeletionDialog(
              ...
              leftAction: AccountDeletionDialogAction(label: 'Zurück', onPressed: () => Navigator.of(ctx, rootNavigator: true).maybePop()),
              rightAction: AccountDeletionDialogAction(
      ...
      await showGeneralDialog<void>(
        ...
        pageBuilder: (ctx, a1, a2) {
          return AccountDeletionDialog(
            ...
            leftAction: AccountDeletionDialogAction(label: 'OK', onPressed: () => Navigator.of(ctx, rootNavigator: true).maybePop()),
            rightAction: AccountDeletionDialogAction(label: 'Schließen', onPressed: () => Navigator.of(ctx, rootNavigator: true).maybePop()),
          );
        },
      );
    } finally {
  ...
  Future<void> _showDeleteBlockedDialog(List<AccountDeletionBlocker> blockers) async {
    if (!mounted) return;
    await showGeneralDialog<void>(
      context: context,
      barrierDismissible: true,
      ...
      pageBuilder: (ctx, a1, a2) {
        return AccountDeletionDialog(
          ...
          leftAction: AccountDeletionDialogAction(label: 'Schließen', onPressed: () => Navigator.of(ctx, rootNavigator: true).maybePop()),
          rightAction: AccountDeletionDialogAction(
...
class _DeleteAccountStep1Body extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final style = t.textTheme.bodyMedium?.copyWith(color: Colors.white70, height: 1.5);
    final bulletStyle = t.textTheme.bodyMedium?.copyWith(color: Colors.white, height: 1.5, fontWeight: FontWeight.w700);
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, mainAxisSize: MainAxisSize.min, children: [
      ...
      _Bullet(text: 'keine laufenden Buchungen bestehen', style: bulletStyle),
      _Bullet(text: 'keine kommenden Buchungen geplant sind', style: bulletStyle),
      _Bullet(text: 'keine laufenden Anmietungen bestehen', style: bulletStyle),
      _Bullet(text: 'keine offenen Zahlungen oder Auszahlungen vorhanden sind', style: bulletStyle),
...
class _DeleteAccountStep2Body extends StatelessWidget {
  ...
  const _DeleteAccountStep2Body({required this.controller, required this.onChanged});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, mainAxisSize: MainAxisSize.min, children: [
      ...
      _Bullet(text: 'deine aktiven Angebote werden entfernt', style: bulletStyle),
      _Bullet(text: 'du kannst dich nicht mehr einloggen', style: bulletStyle),
      _Bullet(text: 'Bewertungen bleiben anonymisiert erhalten', style: bulletStyle),
      const SizedBox(height: 12),
      ...
      Container(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            ...
            prefixIcon: Icon(enabled ? Icons.check : Icons.lock_outline, color: enabled ? t.colorScheme.primary : Colors.white70),
      ...
      Text('Optional (später): Passwort oder E-Mail-Code als zusätzliche Sicherheitsstufe.', style: t.textTheme.labelSmall?.copyWith(color: Colors.white60, height: 1.4)),
    ]);
  }
}
...
class _DeleteBlockedBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: 10),
      for (final b in blockers.take(6)) _Bullet(text: b.label, style: bulletStyle),
      if (blockers.length > 6)
        Padding(
          padding: const EdgeInsets.only(top: 6),
          child: Text('… und weitere', style: t.textTheme.labelSmall?.copyWith(color: Colors.white60)),
        ),
      const SizedBox(height: 12),
    ...
    return Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Padding(
          ...
          child: SizedBox(width: 6, height: 6, child: DecoratedBox(decoration: BoxDecoration(color: Colors.white70, shape: BoxShape.circle))),
        ),
