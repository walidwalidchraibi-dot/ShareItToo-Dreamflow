...
| `TD-RR-010` | S4X replaces the Flutter analyzer's permissive ceiling with an exact normalized diagnostic fingerprint. S4Y through S4AG guard Wishlist selector, popup navigator, gallery callback, item-card state, listing-options callback, profile-screen, listing-photo and public-profile menu lifetimes, ratcheting the backlog `220 -> 214 -> 212 -> 210 -> 207 -> 204 -> 203 -> 202 -> 200`; `use_build_context_synchronously` decreases `98 -> 92 -> 90 -> 88 -> 85 -> 82 -> 81 -> 80 -> 78`, most recently at `4f8a150`. | Continue reducing the committed snapshot only alongside reviewed source fixes until total zero, then retain green exact-commit CI. Never raise it, replace findings at equal count, suppress lints, make warnings non-fatal or update evidence merely to pass. |
