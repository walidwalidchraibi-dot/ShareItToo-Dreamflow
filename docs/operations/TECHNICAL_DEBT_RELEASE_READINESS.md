...
| `TD-RR-003` | S4S adds a clean-exact-commit proof that runs the complete Flutter suite five times at default parallelism and rejects concurrency overrides, sleeps and retries. All five local runs passed with 379 tests plus one documented skip each at `cea3a1f`; exact-commit CI with the explicit stress input is unavailable. During the later S4AS clean-head run, output stopped after 293 green results while the Flutter compiler was idle; after a requested terminal interrupt, the same command continued and returned success. That intervened run is not acceptance evidence. | Retain green exact-commit CI with `run_flutter_parallel_stress=true` using the same five complete runs. No concurrency one, reduced suite, sleep, retry, terminal intervention or pass-on-rerun may appear in the evidence. |
...
| `TD-RR-010` | S4X replaces the Flutter analyzer's permissive ceiling with an exact normalized diagnostic fingerprint. S4Y through S4AS guard Wishlist selector, popup navigator, gallery callback, item-card state, listing-options callback, profile-screen, listing-photo, public-profile menu, request-detail navigation, renter-bookings, owner-requests, search-results, Explore, message-thread, every owner-/booking-detail async-context lifetime and item-detail reservation completion. The backlog ratchets `220 -> 214 -> 212 -> 210 -> 207 -> 204 -> 203 -> 202 -> 200 -> 198 -> 196 -> 194 -> 191 -> 188 -> 182 -> 175 -> 171 -> 165 -> 155 -> 143 -> 132`; `use_build_context_synchronously` decreases `98 -> 92 -> 90 -> 88 -> 85 -> 82 -> 81 -> 80 -> 78 -> 76 -> 74 -> 72 -> 69 -> 66 -> 60 -> 53 -> 49 -> 43 -> 33 -> 21 -> 10`, most recently at `be95424`. Owner-detail and booking-detail are zero; item-overlay has 10 context findings. S4AS also removes three 120-millisecond and one 80-millisecond reservation-navigation waits without replacement. | Continue reducing the committed snapshot only alongside reviewed source fixes until total zero, then retain green exact-commit CI. Never raise it, replace findings at equal count, suppress lints, make warnings non-fatal or update evidence merely to pass. |
