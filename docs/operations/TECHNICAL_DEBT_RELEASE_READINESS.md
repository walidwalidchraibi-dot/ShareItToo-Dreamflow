...
| `TD-RR-010` | S4X replaces the Flutter analyzer's permissive ceiling with an exact normalized diagnostic fingerprint. S4Y through S4AX guard Wishlist selector, popup navigator, gallery callback, item-card state, listing-options callback, profile-screen, listing-photo, public-profile menu, request-detail navigation, renter-bookings, owner-requests, search-results, Explore, message-thread, every owner-/booking-detail async-context lifetime, every item-detail async boundary, all item-detail radio ownership and the item-detail, Explore and DataService dead-code boundaries. The backlog ratchets `220 -> 214 -> 212 -> 210 -> 207 -> 204 -> 203 -> 202 -> 200 -> 198 -> 196 -> 194 -> 191 -> 188 -> 182 -> 175 -> 171 -> 165 -> 155 -> 143 -> 132 -> 122 -> 86 -> 71 -> 59 -> 55`; `use_build_context_synchronously` decreases `98 -> 92 -> 90 -> 88 -> 85 -> 82 -> 81 -> 80 -> 78 -> 76 -> 74 -> 72 -> 69 -> 66 -> 60 -> 53 -> 49 -> 43 -> 33 -> 21 -> 10 -> 0`, and `deprecated_member_use` decreases `36 -> 0`. Owner-detail, booking-detail, item-overlay, Explore and DataService findings are zero, most recently at `0fcf3dd`. S4AS also removes three 120-millisecond and one 80-millisecond reservation-navigation waits without replacement. | Continue reducing the committed snapshot only alongside reviewed source fixes until total zero, then retain green exact-commit CI. Never raise it, replace findings at equal count, suppress lints, make warnings non-fatal or update evidence merely to pass. |
