...
| `TD-RR-010` | S4X replaces the Flutter analyzer's permissive ceiling with an exact normalized diagnostic fingerprint. S4Y through S4AP guard Wishlist selector, popup navigator, gallery callback, item-card state, listing-options callback, profile-screen, listing-photo, public-profile menu, request-detail navigation, renter-bookings, owner-requests, search-results, Explore, message-thread and all owner-detail async-context lifetimes, ratcheting the backlog `220 -> 214 -> 212 -> 210 -> 207 -> 204 -> 203 -> 202 -> 200 -> 198 -> 196 -> 194 -> 191 -> 188 -> 182 -> 175 -> 171 -> 165`; `use_build_context_synchronously` decreases `98 -> 92 -> 90 -> 88 -> 85 -> 82 -> 81 -> 80 -> 78 -> 76 -> 74 -> 72 -> 69 -> 66 -> 60 -> 53 -> 49 -> 43`, most recently at `921d185`. Owner-detail is now zero; booking-detail and item-overlay remain. | Continue reducing the committed snapshot only alongside reviewed source fixes until total zero, then retain green exact-commit CI. Never raise it, replace findings at equal count, suppress lints, make warnings non-fatal or update evidence merely to pass. |
