...
| `TD-RR-003` | S4S adds a clean-exact-commit proof that runs the complete Flutter suite five times at default parallelism and rejects concurrency overrides, sleeps and retries. All five local runs passed with 379 tests plus one documented skip each at `cea3a1f`; exact-commit CI with the explicit stress input is unavailable. During S4AS, output stopped after 293 green results while the Flutter compiler was idle; after a requested terminal interrupt, the same command continued and returned success, so it is not acceptance evidence. At S4AT `1a41bc4`, a new unchanged default-parallel clean-head run again stopped after 293 green results, this time with no test worker and no output for more than six minutes; it was terminated and recorded as failed. A diagnostic-only serial run then passed 384 tests plus one documented skip, narrowing the local fault to the parallel compiler/worker path without changing a gate. Exact S4AT CI run `32600955120` passed at default parallelism. At S4AU `618916e`, the unchanged local default-parallel gate stopped after 192 green results with the same idle compiler/no-worker state and was terminated after more than six silent minutes; no serial rerun was accepted. | Retain green exact-commit CI with `run_flutter_parallel_stress=true` using the same five complete runs. No concurrency one, reduced suite, sleep, retry, terminal intervention, termination or pass-on-rerun may appear in the evidence. |
...
| `TD-RR-010` | S4X replaces the Flutter analyzer's permissive ceiling with an exact normalized diagnostic fingerprint. S4Y through S4AW guard Wishlist selector, popup navigator, gallery callback, item-card state, listing-options callback, profile-screen, listing-photo, public-profile menu, request-detail navigation, renter-bookings, owner-requests, search-results, Explore, message-thread, every owner-/booking-detail async-context lifetime, every item-detail async boundary, all item-detail radio ownership and the item-detail and Explore dead-code boundaries. The backlog ratchets `220 -> 214 -> 212 -> 210 -> 207 -> 204 -> 203 -> 202 -> 200 -> 198 -> 196 -> 194 -> 191 -> 188 -> 182 -> 175 -> 171 -> 165 -> 155 -> 143 -> 132 -> 122 -> 86 -> 71 -> 59`; `use_build_context_synchronously` decreases `98 -> 92 -> 90 -> 88 -> 85 -> 82 -> 81 -> 80 -> 78 -> 76 -> 74 -> 72 -> 69 -> 66 -> 60 -> 53 -> 49 -> 43 -> 33 -> 21 -> 10 -> 0`, and `deprecated_member_use` decreases `36 -> 0`. Owner-detail, booking-detail, item-overlay and Explore findings are zero, most recently at `3df03c0`. S4AS also removes three 120-millisecond and one 80-millisecond reservation-navigation waits without replacement. | Continue reducing the committed snapshot only alongside reviewed source fixes until total zero, then retain green exact-commit CI. Never raise it, replace findings at equal count, suppress lints, make warnings non-fatal or update evidence merely to pass. |
...
| `TD-RR-012` | During S4AV, the first focused standard-parallel Flutter selection failed after 15 tests because the macOS data volume had only 238 MiB free. Only regenerable Flutter, Homebrew, pip and pnpm caches were removed; the identical 96-test command and then the complete standard gate passed at `4632aac`. This recovery is valid local hygiene but cannot become a recurring test prerequisite. | Retain a clean exact-commit release-host run with recorded adequate capacity, bounded temp/build growth and the unchanged complete gate. Manual cache purge, alternate temp root, smaller suite, serial execution, retry or pass-on-rerun cannot be release-readiness evidence. |
