...
test('R16 binds the current 69-up 42-down migration inventory', () => {
  assert.equal(evidence.migrationInventory.orderedUpScripts, 69);
  assert.equal(evidence.migrationInventory.pairedDownScripts, 42);
  assert.equal(evidence.migrationInventory.forwardOnlyRange, '001-027');
  assert.equal(evidence.migrationInventory.pairedRange, '028-069');
});
