...
const source = readFileSync(
  new URL('../../lib/screens/search_results_screen.dart', import.meta.url),
  'utf8',
);
...
assert.ok(favoriteAction, 'expected search-results favorite action');

test('search favorite flow rechecks lifecycle after wishlist lookup', () => {
  assert.match(
    favoriteAction,
...
test('search favorite move rechecks lifecycle after option selection', () => {
  assert.match(
    favoriteAction,
...
test('search-results lifecycle fix contains no timing or lint accommodation', () => {
